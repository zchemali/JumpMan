...
public class Game extends Canvas implements Runnable {
	...
	public Game() {
		...
		window = new Window(WIDTH, HEIGHT, this,backGround);// calling window class
	...
	public void update() {
		...
		for (int i = 0; i < handler.gameObjects.size(); i++) {
			if (handler.gameObjects.get(i).tag == Tag.Player) {
				if(handler.gameObjects.get(i).getHealth()<1)
				{	
		...
		for (int xx = 0; xx < h; xx++) {
			for (int yy = 0; yy < w; yy++) {
				...
				// light grey vlocks
					...
					handler.addObject(new Player(xx * 32, yy * 32 - 90, Tag.Player, texture,this));
